...
def match_single_entry(term, group):
    ...
    def get_best_match(norm_term, group_name):
        ...
        # Step 3: FAISS + Rerank fallback only if top match is weak and not subset match
        top_subset_match = next((c[0] for c in candidates[:5] if set(normalize_string(c[0]["description"]).split()).issubset(tokens)), None)
        ...
        "match_reason": f"token-subset-{group_name}"
            ...
            query_vec = embed_model.encode([norm_term]).astype("float32")
            query_vec = normalize(query_vec, norm='l2')
